...
export class CommissionsComponent implements OnInit {
  ...
  public searchCommission(
    ...
    let start = this.dateFormCommission
      ? this.dateFormCommission.controls.dateRange.value.startDate.format(
          'YYYY-MM-DD'
        )
      : moment(new Date(2020, 0, 1)).format('YYYY-MM-DD');
    let end = this.dateFormCommission
      ? this.dateFormCommission.controls.dateRange.value.endDate.format(
          'YYYY-MM-DD'
        )
  ...
  private setValueAdditionalInfo(commission) {
    ...
    for (let index = 0; index < keysCommission.length; index++) {
      ...
      for (
        let index2 = 0;
        index2 < this.dataAdditionalInfoValue.length;
        index2++
      ) {
        if (
          this.dataAdditionalInfoValue[index2].info.some((x) => x.code === key)
        ) {
          this.dataAdditionalInfoValue[index2].info.find(
            (x) => x.code === key
  ...
  handleFileInput(event, typeUpload: string) {
    ...
    if (this.validFormat(file[0].name)) {
      ...
      formDataCommission.append(
        'formFile',
        file[0],
        file[0].name.replace(' ', '_')
      );

      if (typeUpload === 'rejected') {
        this.subscription = this.link
          .updateStatusCommissionFile(formDataCommission)
          .subscribe((resp: ResponseService) => {
            let text, type;
            if (resp.state === 'Success') {
              text =
                'Se ha subido exitosamente el archivos de comisiones rechazadas.';
              type = 'success';
            } else {
        ...
        this.subscription = this.link
          .deleteCommissionFile(formDataCommission)
          .subscribe((resp: ResponseService) => {
            let text, type;
            if (resp.state === 'Success') {
              text =
                'Se ha subido exitosamente el archivos de comisiones eliminadas.';
              type = 'success';
            } else {
      ...
      this.openSnackBar(
        'Formato inválido solo se pueden subir archivos con la extensión xlsx, xls',
        'Cerrar'
      );
    }
  }
